...
# This is an implementation of a Bayesian classifier combination neural network - 
# a neural network that is trained directly from noisy crowdsourced labels, 
# it jointly aggregates crowdsourced labels removing biases and identifying reliability of each 
# crowd member and learns weights of a neural network, which then can be used to label new data.

# from: https://github.com/OlgaIsupova/BCCNet
...
def expected_true_labels(X, nn_output, ElogPi_volunteer):
    ...
    for w in range(W):
        inds = np.where(X[:, w] > -1)[0].astype(int)
        rho[inds, :] = rho[inds, :] + np.transpose(ElogPi_volunteer[:, np.squeeze(X[inds, w]).astype(int), w])
    ...
    for w in range(W):
        for l in range(L):
            inds = np.where(X[:, w] == l)[0].astype(int)
